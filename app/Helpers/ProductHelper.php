    ...
    $combinedDataProducts = DB::table('master_product_fgs')
      ...
      ->unionAll(
        DB::table('master_raw_materials')
          ->select('id', 'rm_code as product_code', 'description', 'id_master_units', DB::raw("'RM' as type_product"))
          ->where('status', 'Active')
      )
      ->unionAll(
        DB::table('master_tool_auxiliaries')
          ->select('id', 'code as product_code', 'description', 'id_master_units', DB::raw("'AUX' as type_product"))
      )
      ->get();
