    ...
    public function index(Request $request)
    {
        if (request()->ajax()) {
            ...
            $query = DB::table('sales_orders as a')
                ...
                ->select('a.id', 'a.id_order_confirmations', 'a.so_number', 'a.date', 'a.so_type', 'b.name as customer', 'c.name as salesman', 'a.reference_number', 'a.due_date', 'a.status', 'a.qty', 'a.qty_results', 'a.outstanding_delivery_qty', 'a.cancel_qty', 'e.product_code', 'e.description', 'f.unit_code', 'e.perforasi', 'e.weight')
    ...
    public function show(salesOrder $salesOrder, $encryptedSONumber)
    {
        ...
        if ($typeProduct == 'WIP') {
            ...
            $product = DB::table('master_product_fgs as a')
                ->select('a.id', 'a.description', 'a.id_master_units', 'a.sales_price as price', 'a.perforasi', 'a.group_sub_code')
                // ->join('master_units as b', 'a.id_master_units', '=', 'b.id')
                ->where('a.id', $idProduct)
                ->first();
        } else if ($typeProduct == 'RM') {
            $product = DB::table('master_raw_materials as a')
                ->select('a.id', 'a.description', 'a.id_master_units', DB::raw('"" as perforasi'))
                // ->join('master_units as b', 'a.id_master_units', '=', 'b.id')
                ->where('a.id', $idProduct)
                ->first();
        ...
        $combinedDataProducts = DB::table('master_product_fgs')
            ...
            ->unionAll(
                DB::table('master_wips')
                    ->select('id', 'wip_code as product_code', 'description', 'id_master_units', DB::raw("'WIP' as type_product"), 'perforasi', 'weight', DB::raw('"" as group_sub_code'))
                    ->where('status', 'Active')
            )
            ->unionAll(
        ...
        if ($order_number != null || $order_number != '') {
            ...
            if ($typeProduct == 'WIP') {
                $detail_product = DB::table('master_wips as a')
                    ->select('a.id', 'a.description', 'a.id_master_units', 'a.perforasi', 'a.weight')
                    // ->join('master_units as b', 'a.id_master_units', '=', 'b.id')
                    ->where('a.id', $idProduct)
                    ->first();
            } else if ($typeProduct == 'FG') {
                $detail_product = DB::table('master_product_fgs as a')
                    ->select('a.id', 'a.description', 'a.id_master_units', 'a.sales_price as price', 'a.perforasi', 'a.weight', 'a.group_sub_code')
                    // ->join('master_units as b', 'a.id_master_units', '=', 'b.id')
                    ->where('a.id', $idProduct)
                    ->first();
            } else if ($typeProduct == 'RM') {
                $detail_product = DB::table('master_raw_materials as a')
                    ->select('a.id', 'a.description', 'a.id_master_units', 'a.weight')
                    // ->join('master_units as b', 'a.id_master_units', '=', 'b.id')
                    ->where('a.id', $idProduct)
                    ->first();
